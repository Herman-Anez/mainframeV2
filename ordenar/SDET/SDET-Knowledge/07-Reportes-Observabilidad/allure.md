...
## 🏗️ Arquitectura de Allure

1.  **Adaptadores (bindings):** Durante la ejecución de las pruebas, una librería específica del lenguaje escribe resultados crudos en formato JSON (`allure-results`). Por ejemplo, en Java con `allure-junit5` o `allure-testng`, en Python con `allure-pytest`.
2.  **Generador de reportes:** Una herramienta de línea de comandos (`allure`) o un plugin de Maven/Gradle toma los resultados JSON y los transforma en un sitio HTML estático (`allure-report`). Este HTML puede servirse localmente (`allure open`) o publicarse en un servidor web.

---

## 🛠️ Integración con pruebas
...
Usando `allure-cucumber-jvm`, los escenarios y sus pasos se reflejan automáticamente. Se pueden añadir `@Severity`, `@Feature` en los features/escenarios.
...
| :--- | :---: | ---: |
| [⬅️ Inicio del Módulo](index.md) | [🏠 Inicio](../../index.md) | [ExtentReports ➡️](extentReports.md) |