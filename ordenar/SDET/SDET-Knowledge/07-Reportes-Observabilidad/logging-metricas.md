...
- **Identificadores de correlación:** Usar (traceId, testCaseId) en ambos lados (app y prueba). El test genera un ID único al inicio y lo pasa en las peticiones (ej. cabecera `X-Correlation-ID`). Así, los logs del backend pueden ser enlazados con la ejecución de la prueba específica.
- **Filtrado:** No saturar los logs de la aplicación; en entornos de prueba se puede activar un nivel DEBUG, pero filtrar por ese ID.
- **Contexto:** Capturar parámetros de entrada y puntos de verificación. Un log antes de cada assert facilita el análisis post-mortem.
...
## 🏗️ ReportPortal: una plataforma integral

[ReportPortal.io](https://reportportal.io) es una solución open-source que unifica reportes multi-lenguaje (JUnit, TestNG, pytest, Cucumber, etc.) en un solo lugar, con análisis de fallos, dashboards y comparaciones.
...
## 🔗 Correlación con logs de la aplicación (Observabilidad real)
...
> [!TIP]
> Un sistema de reportes y observabilidad maduro transforma los datos crudos de las ejecuciones en conocimiento accionable, permitiendo al equipo reaccionar rápido y mantener la confianza en la automatización.

---

| Anterior | Inicio | Siguiente |
| :--- | :---: | ---: |
| [⬅️ ExtentReports](extentReports.md) | [🏠 Inicio](../../index.md) | [Herramientas Complementarias ➡️](../08-Herramientas-Complementarias/Swagger-OpenAPI.md) |