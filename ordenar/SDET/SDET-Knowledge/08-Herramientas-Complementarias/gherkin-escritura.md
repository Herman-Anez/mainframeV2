...
## 🏗️ Principios de un buen escenario

### 1. Declarativo, no imperativo
Un escenario debe describir qué comportamiento se espera, no cómo se implementa la interacción.
...
Cada escenario verifica una única regla de negocio. Si falla, debe ser obvio por qué. Evitar largas secuencias de acciones mezcladas con múltiples aserciones.

### 3. Independencia
Los escenarios no deben depender unos de otros. Deben poder ejecutarse en cualquier orden. Las dependencias crean fragilidad y falsos positivos.

### 4. Datos relevantes y contextuales
Usar **Background** para pasos comunes, y **Scenario Outline** para ejecutar el mismo escenario con múltiples conjuntos de datos usando Examples.
...
Los archivos `.feature` se versionan junto al código de prueba. El SDET configura el runner para que falle el build si existen pasos sin implementar (`@Pending`). Los reportes Cucumber (integrándose con Allure o usando el reporte HTML de Cucumber) dan visibilidad al negocio de los escenarios que no pasan.

### Ejemplo completo de ciclo de un escenario

1.  **Negocio:** "Quiero que un usuario no pueda comprar si no tiene saldo".
2.  **Refinamiento:** Se crea el escenario:
    ```gherkin
    ...
    ```
3.  **Implementación:** El SDET desarrolla los steps: `Given un cliente con saldo...` que prepara datos (API o BD), `When intenta pagar` que invoca el endpoint de pago, y los `Then` que validan la respuesta.
4.  **Resultado:** La prueba pasa en CI y el PO ve en el reporte el escenario verde.

---

| Anterior | Inicio | Siguiente |
| :--- | :---: | ---: |
| [⬅️ Jira y TestRail](Jira-TestRail.md) | [🏠 Inicio](../../index.md) | [Proyectos de Ejemplo ➡️](../09-Proyectos-Ejemplo/index.md) |
