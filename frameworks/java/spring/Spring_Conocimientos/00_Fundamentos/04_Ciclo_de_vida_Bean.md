...
1. Método anotado con **`@PostConstruct`** (detectado por el `CommonAnnotationBeanPostProcessor` que se ejecutó antes).
2. **`afterPropertiesSet()`** de la interfaz `InitializingBean`.
3. Método **`init-method`** personalizado definido en `@Bean(initMethod = "nombre")` o en XML.
...
- Se ejecuta `postProcessAfterInitialization(bean, beanName)`. Esta es la etapa donde normalmente se generan los **proxies** (AOP, transacciones, seguridad). 
- Si el bean necesita ser envuelto en un proxy, el **`AbstractAutoProxyCreator`** (un `BeanPostProcessor`) reemplaza la instancia original por un proxy CGLIB o JDK.

> [!WARNING]
> Si llamas a un método interno dentro del mismo bean, la anotación `@Transactional` no se aplica porque la llamada no pasa por el proxy.
...
- **`destroy()`** de la interfaz `DisposableBean`.
- Método **`destroy-method`** personalizado de `@Bean` o XML.
- Los `DestructionAwareBeanPostProcessor` pueden ejecutar lógica previa.

## Diagrama resumido (flujo)

```text
[Constructor o Fábrica] 
   --> [Inyección de Deps] 
      --> [Aware: BeanName, ApplicationContext, etc.]
         --> [BeanPostProcessor::before] 
            --> [@PostConstruct / afterPropertiesSet / init-method]
               --> [BeanPostProcessor::after] (proxies creados aquí) 
                  --> [Bean listo]
...
### BeanFactoryPostProcessor y BeanDefinitionRegistryPostProcessor

Antes de que ningún bean sea instanciado, el contenedor permite modificar las propias definiciones de los beans. 
- Los **`BeanFactoryPostProcessor`** trabajan con el `BeanFactory` (en realidad `ConfigurableListableBeanFactory`). 
- Los **`BeanDefinitionRegistryPostProcessor`** pueden incluso registrar nuevas definiciones de beans.
...
- Facilita la **depuración** de problemas de beans: si una dependencia se resuelve mal, sabrás en qué fase mirar.
