# Abstracción de Caché en Spring
...
Spring Boot autoconfigura un `CacheManager` según las dependencias detectadas:

1.  **Caffeine** (recomendada para caché local) con `spring-boot-starter-cache`.
2.  **Redis** con `spring-boot-starter-data-redis`.
3.  **EhCache 3**, **Hazelcast**, etc.

### Configuración con Caffeine

Basta añadir la dependencia y configurar en `application.properties`:
...
Si necesitas múltiples caches con configuraciones distintas, puedes definir un bean de `CacheManager`:
...
## Caché a Nivel de Anotaciones Personalizadas
...
## Caché con Spring WebFlux (Reactivo)
