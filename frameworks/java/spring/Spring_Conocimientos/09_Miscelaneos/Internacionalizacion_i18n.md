...
1.  **ResourceBundleMessageSource:** Carga bundles `.properties` desde el classpath. Sin caché configurable (lee cada vez por defecto, aunque internamente usa `ResourceBundle` con caché de la JVM).
2.  **ReloadableResourceBundleMessageSource:** Similar, pero soporta recarga en caliente sin reiniciar la aplicación. Ideal para desarrollo o cuando los bundles están externos.
3.  **StaticMessageSource:** Para mensajes programáticos, útil en tests.
...
*   **Centralización:** Centraliza los mensajes en archivos `.properties` con nombres descriptivos.
*   **Desarrollo:** Usa `ReloadableResourceBundleMessageSource` en desarrollo para recarga en caliente.
*   **Simplicidad:** Evita mensajes largos con lógica de negocio en las plantillas; mantenlos simples.
*   **Escalabilidad:** Para aplicaciones con muchos idiomas, considera servicios externos de traducción o un CMS.

---

## Apéndice: Referencia Detallada
...
### Configuración Avanzada del Locale

Para permitir al usuario cambiar de idioma dinámicamente:
...
### Integración con Thymeleaf y REST
