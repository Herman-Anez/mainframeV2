# 📁 Cuantificadores: Ejemplos Prácticos

A continuación, se presentan ejemplos que muestran la aplicación real de cuantificadores codiciosos, perezosos y posesivos, con explicaciones detalladas.
...
- **Greedy incorrecto:** `<strong>.*</strong>`
    ...
    - El `.*` consume todo hasta el último `</strong>`. No es adecuado si queremos capturas individuales.
...
**Requerimiento:** Dígitos, posibles guiones o espacios como separadores, longitud total controlada.
...
> [!TIP]
> Funciona bien con greedy, pero puede tener backtracking con entradas como `"123-456-7890"` que son válidas. Si se aplica un cuantificador posesivo: `^[\d]++([\s-]?+[\d]++)*+$` se optimiza y asegura que no retroceda en dígitos (sólo en motores con soporte).
...
**Explicación:**
1. `"` comilla inicial.
2. `([^"\\]|\\.)*?` cero o más veces (perezoso) de:
...
## 📌 4. Log parsing: extracción de direcciones IP
...
1. `^` inicio de línea.
2. `\S+` uno o más caracteres no espacio (greedy, pero como `\S` no puede cruzar espacios, no hay riesgo).

> [!NOTE]
> Si hubiese riesgo de backtracking, podríamos usar posesivo `\S++` en motores que lo permitan, pero no es necesario porque la clase negada no puede coincidir con un espacio.
...
**Patrón perezoso:** `/\*.*?\*/` funciona porque `.*?` se expande hasta encontrar `*/`. Requiere la flag `s` (DOTALL) para que `.` incluya saltos de línea.

Sin embargo, puede haber problemas si dentro del comentario aparece un asterisco solitario.
**Mejor patrón:** `/\*[^*]*\*+(?:[^/*][^*]*\*+)*/` (patrón eficiente para comentarios C). Aquí usamos `*` y `+` greedy, pero estructurados para no saltar delimitadores.
...
Un patrón simple recursivo (en PCRE): `\(([^()]|(?R))*\)` es greedy normal y puede sufrir backtracking con cadenas largas no balanceadas.
**Optimización:** `\(([^()]++|(?R))*+\)` usando posesivos. Esto restringe retrocesos inútiles dentro de la repetición de caracteres no paréntesis.
...
import re
texto = '"primero" y "segundo"'
re.sub(r'"[^"]*"', 'X', texto)  # Clase negada: a veces mejor que lazy, más seguro.
```
...
Para palabras de al menos 4 letras: `\b\w{4,}\b` el greedy consume toda la palabra, que es lo deseado.

---

## 📌 9. Backtracking catastrófico y cómo solucionarlo

**Ejemplo de patrón vulnerable:** `(\d+,)*\d+` (validación de número de serie con partes opcionales).
- **Entrada:** `"123,456,789"` funciona bien.
- **Entrada malformada:** `"123,456,789,"` puede causar backtracking intenso.
...
En PCRE podemos usar condicionales con cuantificadores.
**Ejemplo:** `^(?:(\())\d+(?(1)\))$` coincide con `(123)` o `123`, pero no `(123`.

Aquí la repetición es `\d+` (greedy), podríamos hacerla posesiva para evitar backtracking: `\d++`.

---

| Anterior | Inicio | Siguiente |
| :--- | :---: | ---: |
| [Cuantificadores Posesivos](02_posesivos.md) | [Índice](../README.md) | [Agrupaciones y Captura](../03_agrupaciones_y_captura/01_grupos_captura.md) |
(greedy), podríamos hacerla posesiva para evitar backtracking: \d++.
